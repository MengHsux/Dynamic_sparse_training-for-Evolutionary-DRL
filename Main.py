...
# Env
parser.add_argument('-env_name', default="Ant-v2", type=str)
...
# for sparse model
parser.add_argument("--save_model_period", default=1e5,
                    type=int)  # Save model and optimizer parameters after the set number of iterations
parser.add_argument("--ann_noHidNeurons", default=256, type=int)  #
parser.add_argument("--ann_epsilonHid1", default=40, type=int)  # lambda 1
...
if __name__ == "__main__":
    ...
    while tot_steps < args.max_steps:
        ...
        a1, f1 = rl_agent.actor(stateOut)
        a2, f2 = actor(stateOut)

        predistOut = copy.deepcopy(((a1 - a2) ** 2).mean().item())
        a11, f11 = rl_agent.actor(stateOut)
        preQ = copy.deepcopy(rl_agent.critic.Q1(stateOut, a11).mean().item())
        ...
        if guided_learning:
            ...
            for k in range(args.dv_num):
                ...
                actor.set_params(es_agent.elite_param)
                a12, f12 = rl_agent.actor(stateIn)
                a22, f22 = actor(stateIn)
                predistIn = copy.deepcopy(((a12 - a22) ** 2).mean().item())
                ...
                a13, f13 = rl_agent.actor(stateIn)
                a23, f23 = actor(stateIn)
                postdistIn = ((a13 - a23) ** 2).mean().item()
        ...
        a14, f14 = rl_agent.actor(stateOut)
        a24, f24 = actor(stateOut)
        postdistOut = ((a14 - a24) ** 2).mean().item()
        a15, f15 = rl_agent.actor(stateOut)
        postQ = rl_agent.critic.Q1(stateOut, a15).mean().item()
        ...
        if not args.q_surr:
            ...
            for param in params:
                ...
                a25, f25 = actor(states)
                q_val = copy.deepcopy(rl_agent.critic.Q1(states, a25).mean().item())
            ...
            for param in expl_params:
                actor.set_params(param)
                a26, f26 = actor(states)
                q_val = copy.deepcopy(rl_agent.critic.Q1(states, a26).mean().item())
                expl_q.append(q_val)
            ...
            actor.set_params(es_agent.elite_param)
            a27, f27 = actor(states)
            q_values[0] = copy.deepcopy(rl_agent.critic.Q1(states, a27).mean().item())
        ...
        if (generation + 1) % args.RtoE == 0:
            ...
            if args.q_surr and tot_steps > args.surr_start:
                a28, f28 = rl_agent.actor(states)
                q_values[-1] = copy.deepcopy(rl_agent.critic.Q1(states, a28).mean().item())
